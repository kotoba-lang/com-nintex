(ns nintex.main
  "Kotodama WASM entrypoint for the Nintex clean-room actor (L4) — Clojure port.
  ...
  facts (`nintex.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "nintex")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"nintex.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "nintex-compat" :tier tier :entities entities} 200])
...
(defn start! [] :nintex-compat/ready)
